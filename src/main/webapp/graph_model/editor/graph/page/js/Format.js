...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		// Text
		mxUtils.write(label2, mxResources.get('shape',null,'Shape'));
		div.appendChild(label2);

		var shapePanel = div.cloneNode(false);
		shapePanel.style.display = 'none';
		this.panels.push(new ShapeFormatPanel(this, ui, shapePanel));
		this.container.appendChild(shapePanel);
		addClickHandler(label2, shapePanel, idx++);
		/*
 ...
 * Adds the label menu items to the given menu and parent.
 */
ShapeFormatPanel = function(format, editorUi, container)
{
	BaseFormatPanel.call(this, format, editorUi, container);
	this.init();
};

mxUtils.extend(ShapeFormatPanel, BaseFormatPanel);
/**
 * Adds the label menu items to the given menu and parent.
 */
ShapeFormatPanel.prototype.init = function()
{
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	var ss = this.format.getSelectionState();
	this.container.appendChild(this.addValue(this.createPanel()));
};

/*
 *
 */
ShapeFormatPanel.prototype.addSelection = function(div,panel)
{
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	...
	div.appendChild(this.createTitle(panel.label));
	var styleSelect = document.createElement('select');
	styleSelect.style.position = 'absolute';
	styleSelect.style.marginTop = '-2px';
	styleSelect.style.right = '10px';
	styleSelect.style.left = '10px';
	for(var i=0; i<panel.panelChoices.length;i++)
	{
		var choice = panel.panelChoices[i];
		var styleOption = document.createElement('option');
		styleOption.setAttribute('value',choice.value);
		styleOption.setAttribute('name',choice.name);
		styleOption.text = choice.name;	
 ...
 *
 */
ShapeFormatPanel.prototype.xmlToJson = function(xml)
{	
 ...
 * 
 */
ShapeFormatPanel.prototype.addValue = function(div)
{
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	...
	if(ss.vertices.length>0)
	{
		...
		if(ajax_content!=null){
			...
			div.appendChild(new_div);
			for(var i=0;i<ajax_content.panelOptions.length;i++)
			{
				var nodePanel = ajax_content.panelOptions[i];
		...
		/*
		$.getJSON(
			"http://192.168.1.110:8080/sjtu/panel/get_node/25", function(json){
				alert(json.name);
		});
		var req = new mxXmlRequest("http://192.168.1.110:8080/sjtu/panel/get_node/25", null, 'GET', true);
		req.setRequestHeaders('Content-Type', 'jsonp');
		req.send();
		var new_div = this.createPanel();
		mxUtils.write(new_div, mxResources.get('nubbbbb',null,req.getText()));
		div.appendChild(new_div);
		*/
	}
	return div;
